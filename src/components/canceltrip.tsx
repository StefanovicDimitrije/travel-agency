...
interface cancelTripProps{
    ...
    handleCancelTrip:(reason:string)=>void;
}

export default function CancelTrip(props:cancelTripProps){
    ...
    function cancelTrip(e:FormEvent){
        e.preventDefault();
        props.handleCancelTrip(reason);
    }
                                ...
                                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    ...
                                    <div className="text-sm text-gray-500">
                                        <form onSubmit={cancelTrip}>
                                            Doing this will remove the trip from view but will keep it in the "Cancelled" page along with the reason why it was cancelled
                                            <textarea onChange={handleReasonChange} id="cancelreason" placeholder="Tell us the reason you are cancelling the trip" name="cancelreason" rows={3} className="text-black font-medium inline-flex items-center justify-center border rounded leading-snug transition duration-150 ease-in-out px-3 py-3 shadow-sm hover:shadow-lg w-full mt-4 mb-2 resize-none" required></textarea>
                                            <div className="max-w-xs mx-auto sm:max-w-none sm:flex sm:justify-center mt-3">
                                                <button name="submit" className="w-full sm:w-auto"><div className="font-medium inline-flex items-center justify-center border rounded-lg leading-snug transition duration-150 ease-in-out px-8 py-3 shadow-lg text-white bg-red-700 hover:bg-red-800 w-full mb-4 sm:w-auto sm:mb-0">Cancel the trip<i className="ri-mail-forbid-line pl-2"></i></div></button>
                                            </div>
                                        </form>
                                    </div>
